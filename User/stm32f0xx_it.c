		...
		switch(Uart1.PackageStatus) {
			...
			case RECV_START:
				switch(Rev) {
					case '\r':
					case '\n':
						switch(Uart1.RxDataBuf[0]) {
							case 'G':	//调节重量参数
								temp = (u16)(Uart1.RxDataBuf[1]-'0')*100 +(Uart1.RxDataBuf[2]-'0')*10+(u16)(Uart1.RxDataBuf[3]-'0');
								Set_Weight_Gap(temp);
								break;
							case 'R':	//去皮重
								Uart1.RxFlag = 1;	//借用变量
								break;
							case 'P':	//控制电机转速，PWM信号输出
								temp = (u16)(Uart1.RxDataBuf[1]-'0')*100 +(Uart1.RxDataBuf[2]-'0')*10+(u16)(Uart1.RxDataBuf[3]-'0');
								if( temp<=120 ) {
									Device.AtomizerCurPWM = temp;
									TIM_SetCompare2(TIM1,90 + Device.AtomizerCurPWM);
									TIM_SetCompare3(TIM1,90+ Device.AtomizerCurPWM);
									//TIM_SetCompare2(TIM1,90);//关闭
									//TIM_SetCompare3(TIM1,90);
									//TIM_SetCompare2(TIM1,210);//满油门
